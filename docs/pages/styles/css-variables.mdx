...
where `x` is color name and `y` is color shade. For example, `--mantine-color-red-6` is `theme.colors.red[6]`.
You can find full default colors list in [colors guide](/theming/colors/#default-colors).
...
- `--mantine-color-{x}-outline-hover` – outline variant border-color on hover
...
argument and returns object with CSS variables divided into three groups:
