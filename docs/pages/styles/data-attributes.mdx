...
`data-disabled` attribute is added to `<button />` element:
...
component `section` element has associated `data-position` attribute which can be
...
You can then use this attribute to apply styles to left and right sections:
...
[Box](/core/box) component supports `mod` prop which allows to add data attributes to
