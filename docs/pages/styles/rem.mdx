...
All Mantine components use `rem` units to apply size styles. `1rem` is considered to be `16px` as it is a default setting in most browsers.
All components scale based on user's browser font-size settings.

`em` units are supposed to be used only in media queries. `1em` is considered to be `16px` as it is a default setting in most browsers.
