...
You can use any valid CSS value in `size` prop, it is used to set `width`, `min-width` and `height`
properties. Note that `size` prop does not control child [icon](/guides/icons) size, you need to
set it manually on icon component. When `size` is a number, value is treated as `px` units and
converted to [rem](/styles/rem) units (1rem = 16px).
...
- `&:disabled` is used to style button when `disabled` prop is set and
  ...
  `<fieldset />` which contains `ActionIcon`).
- `&[data-disabled]` is used to style button when it is not actually disabled but should look like
  ...
  or when `ActionIcon` is used as link)
