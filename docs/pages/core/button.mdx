...
`leftSection` and `rightSection` allow to add icons or any other element to the left and right side of the button.
...
`justify` prop sets `justify-content` of `inner` element. You can use it to change alignment of
left and right sections. For example, to spread them across the button set `justify` to `space-between`.
...
When `variant` prop is set to `gradient`, you can control button gradient with `gradient` prop,
it accepts an object with `from`, `to` and `deg` properties. If gradient property is not set
`Button` will use `theme.defaultGradient` which can be configured on [theme object](/theming/theme-object).
`gradient` prop is ignored when `variant` is not `gradient`.

Note that `variant="gradient"` supports only linear gradients with two colors. If you need more
complex gradient, then use [Styles API](/styles/styles-api) to modify `Button` styles.
...
rendered as `<a />` set `data-disabled` attribute instead and prevent default behavior in
...
- `&:disabled` is used to style button when `disabled` prop is set and
  also when button is disabled by parent component (for example, when `disabled` prop is set on
  `<fieldset />` which contains `Button`).
- `&[data-disabled]` is used to style button when it is not actually disabled but should look like
  it is (for example, `data-disabled` should be used if you need to use [Tooltip](/core/tooltip) with disabled `Button`
  or when `Button` is used as link)
...
You can customize styles of any inner element inside `Button` with [Styles API](/styles/styles-api)
