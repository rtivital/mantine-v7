...
Most of the icons libraries support `size` prop (or similar `width` and `height` props) which allows to change
...
`currentColor` in `fill` and `stroke` prop. This will change icon color based on the context
it is used in.
