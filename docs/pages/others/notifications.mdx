...
Install `@mantine/notifications` package. Note that the package depends on `@mantine/core` and `@mantine/hooks` packages, you need to install them as well.
To do that, follow the installation instructions on the [getting started page](/getting-started/).
