...
they accept color in any format as first argument and amount of lightness to add or remove as second argument.
...
`parseThemeColor` function returns information about given color in the following format:
...
`getThemeColor` is a simpler version of `parseThemeColor` function, it accepts color string
