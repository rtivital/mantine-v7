...
`withGlobalStyles` determines whether global styles should be added to the document. By default, it is set to `true`, 
you should not change it unless you want to manage your global styles manually.
This can be useful when working with micro front-ends where mantine does not control the entire document.
.
...
function Demo() {
  return (
    <MantineProvider withGlobalStyles={false}>
      <App />
    </MantineProvider>
  );
}
